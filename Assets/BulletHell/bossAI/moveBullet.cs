...
public class moveBullet : MonoBehaviour
{
    ...
    [SerializeField]
    public IEnumerator destroyCondition;
    bool destroying;

    // Start is called before the first frame update
    private void Start()
    {
        _poolManager = GameObject.Find("GameManager").GetComponent<poolManager>();
    }
    void OnEnable()
    {
        destroying = false;
    ...
    void Update()
    {
        if(destroyCondition != null && !destroying)
        {
            StartCoroutine(destroyCondition);
            destroying = true;
        }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if(collision.gameObject.GetComponent<PlayerHealth>() != null)
        {
            ...
            if (removeWhenHit) {
                ...
                _poolManager.unrequestBullet(gameObject);
            }
        }
    }
}
