...
public class bossAttack: MonoBehaviour
{
    ...
    int phase;
    [SerializeField]
    bool periodicAttacks;

    // Start is called before the first frame update
    void Start()
    {
        PoolManager = GameObject.Find(poolManagerName).GetComponent<poolManager>(); //referance for data storage pool manager
        currentPhase = pickAttack(); //Automatically assigns the attack to the first one
        if (periodicAttacks)
        {
    ...
    */
    ...
    IEnumerator spawnGuns(attackBasic attackInfo)
    {
        for(int i = 0; i < attackInfo.spawnInfo.gunInfo.Count; i++) //Spawns a gun for each gun in the attacks's list
        {
            ...
            if (attackInfo.spawnInfo.gunInfo[i].followsLocation)
            {
                StartCoroutine(FollowLocation(gunSpawned, attackInfo.spawnInfo.gunInfo[i].spawnLocation));
            }
    ...
    IEnumerator FollowLocation(GameObject object1, GameObject object2)
    {
            ...
            if(gunSource != null) //Another null check
            {
                for(int j = 0; j < attackInfo.multiShootConfig.bulletMultiplier; j++) //Iterates for each bullet stream
                {
                    ...
                    if (attackInfo.beam)
                    {
                        ...
                        IEnumerator condition = destroyBullet(shootBullet, attackInfo.beamConfig.lifespan, gunSource, kil); //Instead of a bullet getting destroyed when they hit walls, beams get destroyed after a period of time
                        shootBullet.GetComponent<moveBullet>().destroyCondition = condition;
                    }
                    ...
                    {
                        IEnumerator condition = destroyBullet(shootBullet, attackInfo.beamConfig.lifespan, gunSource); //Instead of a bullet getting destroyed when they hit walls, beams get destroyed after a period of time
                        shootBullet.GetComponent<moveBullet>().destroyCondition = condition;
                    }
    ...
    public GameObject createBullet(GameObject shootBullet, BulletSimple attackInfo, GameObject gunSource, shatterShotConfig shatterConfig = null)
    {
        ...
        shootBullet.GetComponent<moveBullet>().damage = attackInfo.bulletDamage;
        shootBullet.GetComponent<moveBullet>().removeWhenHit = attackInfo.removeHit;
    ...
    IEnumerator destroyBullet(GameObject shootBullet, float time, GameObject gunSource, bool kil = false)
    {
        ...
        if((shootBullet != null || shootBullet.activeSelf) && shootBullet.GetComponent<moveBullet>().shootsWhenDie && shootBullet.GetComponent<moveBullet>().dieProperties != null)
        {
        ...
        if(shootBullet != null || shootBullet.activeSelf)
        {
    ...
    {
        List<attackBasic> attackBasicList = phaseAttacksList[phase].attackBasicList;
        attackBasic pickedAttack = attackBasicList[index];
        IEnumerator attackCoroutine = null;
        switch (pickedAttack.Type) //Runs the coroutine that corresponds to the correct attack
        {
            case attackBasic.attackType.shootDown:
                attackCoroutine = shootBullets(pickedAttack);
                break;
            case attackBasic.attackType.wait:
                break;
            case attackBasic.attackType.summonGun:
                attackCoroutine = spawnGuns(pickedAttack);
                break;
        }
